...
console.log(tools)
console.log(typeof tools.spriteFactory);
...
io.on('connection', function(socket) {
    ...
    fs.readFile('./img/delmar.png', function(err, buf) {
        if (err) {
            console.log("errorrrrrr");
            sprites[usernum] = tools.spriteFactory(usernum, delmar);
            io.sockets.emit('newsprite', { num: usernum, sprite: sprites[usernum] });
        } else {
            sprites[usernum] = tools.spriteFactory(usernum, buf.toString('base64'));
            io.sockets.emit('newsprite', { num: usernum, sprite: sprites[usernum] });
        }
    });
